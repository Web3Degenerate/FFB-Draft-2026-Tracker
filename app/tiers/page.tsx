...
export default function TierEditorPage() {
  ...
  const setBenchTargetPlayer = async (player: Player, included: boolean) => {
    setSavingId(player.id);
    try {
      const result = await jsonFetch<{ state: DraftState }>("/api/action", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "set-bench-target-player", playerId: player.id, included }) });
      setPayload((current) => current ? { ...current, state: result.state } : current);
      setNotice({ kind: "success", text: included ? `${player.name} added as a Bench Target.` : `${player.name} removed from Bench Targets.` });
    } catch (error) {
      setNotice({ kind: "error", text: error instanceof Error ? error.message : "Bench Target update failed" });
    } finally { setSavingId(0); }
  };
  ...
  return <main className="management-shell">
    <header className="management-header">
      <div><nav className="management-nav-links"><Link href="/" className="back-link"><ArrowLeft /> Auction Room</Link><Link href="/watch-list" className="back-link"><Eye /> Watch List</Link><Link href="/player-data" className="back-link">Player Data</Link><Link href="/schedules" className="back-link">Schedules</Link></nav><span className="eyebrow">PLAYER CLASSIFICATION</span><h1>Tier Editor</h1><p>Override calculated tiers, set the order inside each tier, and choose players for your positional Watch List or position-agnostic Bench Targets.</p></div>
      <div className="management-summary"><span><strong>{state.watchList.length}</strong><small>watched</small></span><span><strong>{state.benchTargets?.length ?? 0}</strong><small>bench targets</small></span><span><strong>{Object.keys(state.tierOverrides).length}</strong><small>overrides</small></span><span><strong>{tierCounts.length}</strong><small>{position} tiers</small></span></div>
    </header>
    ...
    <section className="tier-editor-panel panel">
      ...
      <div className="tier-editor-table-wrap"><table className="tier-editor-table"><thead><tr><th>{editingTier ? "DRAG" : "RK"}</th><th>PLAYER</th><th>CALCULATED</th><th>MANUAL TIER</th><th>WATCH</th><th>BENCH TARGET</th><th>STATUS</th><th /></tr></thead><tbody>{filtered.map((player, orderIndex) => {
